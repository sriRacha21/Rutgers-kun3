...
module.exports = class Command extends Commando.Command {
    ...
    async run( msg, { role, message }) {
        ...
        if ( !defaultSettings || !defaultSettings.agreementSetupSlimEmote ) { return msg.reply('There are no default settings! Add a `defaultSettings.json` into the settings folder and give it a `agreementSetupSlimEmote` field with the value being the ID for the emote you want to use for the emote reaction.'); }
        ...
        message.react(emote)
            .then(mr => {
                this.client.provider.set( msg.guild, 'agreementSlim', {
                    ...
                    .then(() => {
                        msg.reply('Agreement message successfully set! You can now delete all the messages in this channel other than the agreement message itself.');
                    });
            })
            .catch(err => {
                if (err) { return msg.reply('I was not able to react to the message. Make sure reactions are allowed in this channel.'); }
            });
    }
};
