...
module.exports = class SetSettingCommand extends Commando.Command {
    ...
    async run( msg, { key, value, guild } ) {
        ...
        if ( valPromise ) {
            valPromise.then( msg.reply(`Setting has been successfully set${guild ? ` for guild ${msg.guild.name}.` : '.'}
\`\`\`key: ${key}\nvalue: ${value}\`\`\``) )
                .catch( e => msg.reply(`Could not set value, \`${value}\`, by key, \`${key}\`, from settings: ${e}`) );
        } else { return msg.reply(`Could not set value, \`${value}\`, by key, \`${key}\`, from settings.`); }
    }
};
