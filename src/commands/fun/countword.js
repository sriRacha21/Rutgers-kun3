...
module.exports = class CountWordCommand extends Commando.Command {
    ...
    async run( msg, { word } ) {
        ...
        if ( arr.map(wordCountInfo => wordCountInfo.word).filter(wordArr => wordArr.includes(word) || word.includes(wordArr) ).length > 0 ) { return msg.channel.send( 'Word cannot be set because you have a similar or the same word already being tracked.' ); }
