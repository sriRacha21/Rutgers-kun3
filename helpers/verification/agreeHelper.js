...
function startAgreementProcess(user, guild, settings, provider, errChannel) {
    ...
    user.send( `Please enter the name of the role you want to add. Roles are: ${agreementRoles.map(agreementRole => agreementRole.name).join(', ')}.` )
        .then( () => {
            // capture the user's user ID so we can continue the conversation. Set that we're at step 1
            settings.set( `agree:${user.id}`, {
                guildID: guild.id,
                step: 1
            });
        })
    ...
    Turn on DM's from server members:`, { files: ['resources/setup-images/instructions/notif_settings.png', 'resources/setup-images/instructions/dms_on.png'] });
                ...
                logger.log('warn', `User is not accepting DM's in guild ${guild.name}.`, err);
            }
        });
}
